/*
  ...
  buffer alone needs 1KB, so display.begin() will fail with "SSD1306
...
void updateLook(unsigned long now) {
  // Scripted gestures take full control of the gaze while active, and
  // suppress the normal random saccades below.
  if (doubleTakeMode) {
...
void drawEye(EyeState &e, int cx, int cy, bool isRight) {
  ...
  display.fillRoundRect(x, yy, w, hh, r, SSD1306_WHITE);
  ...
  if (e.squint > 1) {
    ...
    if (sq > 1) {
      ...
      display.fillCircle(cx, ycenter, (int)R, SSD1306_BLACK);
    }
  }
  ...
  if (e.slantInner > 1) {
    ...
    if (isRight) {
      display.fillTriangle(x, yy, x + cutW, yy, x, yy + cutH, SSD1306_BLACK);
    } else {
      display.fillTriangle(x + w, yy, x + w - cutW, yy, x + w, yy + cutH, SSD1306_BLACK);
    }
  }
